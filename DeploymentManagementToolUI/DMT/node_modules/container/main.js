...
require.config({
    "paths": {
        "containerComponents": "../node_modules/container/containerComponents",
        "components/LogoutButton": "../node_modules/container/components/LogoutButton",
        "config": "../container.config"
    }
});


define('text!container/container.html',[],function () { return '<div class=\'eb404Page\'>\n    <div class=\'eb404Page-boxHolder\'>\n        <div class=\'eb404Page-textBox\'>\n            <div class=\'eb404Page-header\'>\n                <span class=\'eb404Page-header-icon\'></span>\n                <h1 class=\'eb404Page-header-title\'>Application Not Found</h1>\n            </div>\n            <div class=\'eb404Page-body\'>Please select Another Application.</div>\n        </div>\n    </div>\n</div>\n';});

define('container/AssetLoader',[],function () {
    ...
    AssetLoader.prototype = {
        ...
        createLink: function (assets) {
            if (this.assets[assets] === undefined) {
                var url = assets + "/css/assets.css",
                    link = document.createElement('link');
                link.setAttribute("rel", "stylesheet");
                link.setAttribute("href", url);
                this.assets[assets] = link;
            }
        },
...
define('container/AppLoader',[
    ...
    AppLoader.prototype = {
        ...
        load: function (appId, breadcrumb) {
            ...
            require([appId + '/config'], function (config) {
                var entryPoint = config.script || appId + "/" + appId;
                var requireConfig = {
                    context: appId,
                ...
                if (config.assets) {
                    requireConfig.paths.assets = 'assets/' + config.assets;
                } else if (config.paths) {
                    config.assets = config.paths.assets;
                }

                if (config.libs) {
                    for (var libName in config.libs) {
                        var libPath = libName + '/' + config.libs[libName];
                ...
                if (config.paths) {
                    for (var path in config.paths) {
                        requireConfig.paths[path] = config.paths[path];
                    }
                }

                require(requireConfig, [entryPoint], function (App) {
                    if (startedLoading === this.startedLoading) {
                        this.onLoad(App, config, appId, breadcrumb);
                    }
                }.bind(this));
            }.bind(this), this.error);
        },

        onLoad: function (App, config, appId, breadcrumb) {
            this.assets.add(config.assets);

            if (this.appId !== undefined && this.appId !== appId) {
                this.apps[this.appId].detach();
            }
            this.appId = appId;
            this.applicationHolder.innerHTML = '';
            if (this.apps[this.appId] === undefined) {
                var app = new App({
...
define('container/AppResolver',[],function () {

    function AppResolver (options) {
        this.error = options.error;
    }

    AppResolver.prototype = {

        resolve: function (location, resolve) {
            var createBreadcrumb = function (apps, breadcrumb, activeSegment, cb) {
                if (apps.length === 0) {
                    cb();
                    return;
                }
                ...
                require([appName + '/config'], function (config) {
                    ...
                    parent.children.push({
                        ...
                        selected: activeSegment === appName
                    });
                    createBreadcrumb(apps, breadcrumb, activeSegment, cb);
                }, this.error);
            }.bind(this);

            var loadConfigJs = function (loaded, toLoad, breadcrumb) {
                if (toLoad.length === 0) {
                    var appName = loaded[loaded.length - 1];
                    resolve(appName, breadcrumb);
                    return;
                }
                ...
                require([current + '/config'], function (config) {
                    ...
                    if (!found) toLoad = [];
                    createBreadcrumb(config.children, breadcrumb, nextSegment, function () {
                        loadConfigJs(loaded, toLoad, breadcrumb);
                    });
                }, this.error);
            }.bind(this);
...
define('container/Container',[
    ...
    Container.prototype = {
        ...
        listenHash: function () {
            ...
            if (location !== '') {
                this.appResolver.resolve(location, function (appName, breadcrumb) {
                    this.appLoader.load(appName, breadcrumb);
                }.bind(this));
            } else {
                this.empty();
            }
...
define('container/SystemBar/Component',[
    'jscore/core',
    'jscore/interfaces'
...
define('text!container/SystemBar/LogoName/_logoName.html',[],function () { return '<div class="eaContainer-SystemBar-name"></div>\n';});

define('styles!container/SystemBar/LogoName/_logoName.less',[],function () { return '.eaContainer-SystemBar-name {\n  display: inline-block;\n  padding: 0 5px 0 0;\n  margin: 0;\n  font-size: 1.4em;\n  line-height: 40px;\n  color: #58585A;\n  max-width: 100%;\n  overflow: hidden;\n  white-space: nowrap;\n  text-overflow: ellipsis;\n}\n';});

/*global define*/
define('container/SystemBar/LogoName/LogoNameView',[
    'jscore/core',
    'text!./_logoName.html',
...
/*global define*/
define('container/SystemBar/LogoName/LogoName',[
    'jscore/core',
    './LogoNameView'
], function (core, View) {
...
define('container/SystemBar/SystemBarComponent',[
    './Component',
    './LogoName/LogoName',
    ...
    return Component.extend({
        ...
        init: function (options, components) {
            ...
            options.name = options.name || 'OSS';

            this.changeTitle(options.name);

            if (options.globalEvents) {
                var globalEvents = options.globalEvents;
                globalEvents.subscribe('titleChange', this.changeTitle);
            }
        },
...
define('container/loader',[
    'config',
    'jscore/core',
    './Container',
    'containerComponents',
    './SystemBar/SystemBarComponent'
], function (config, core, Container, components, SystemBarComponent) {

    var items = [];
    components.list.forEach(function (item) {
        items.push(item.path);
    });

    // for (var key in compileConfig) {
    //     config[key] = compileConfig[key];
    // }
