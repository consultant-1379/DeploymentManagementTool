define([
    ...
    return{
         ...
         */
        svgWidth: function(){
            if ((window.innerWidth - 300 - 185) < 670) {
                return 670;
            }
            return window.innerWidth - 300 - 185;
        },
        /**
        ...
        /**
         * Builds JSON path to be used for RESTFul calls to the DMTService
         * @param pathText - i.e. "/path/subfolder/"
         * @return pathArray - i.e. "children.path.children.subfolder.children"
         */
        getJSONPath: function(pathText){
            var pathArray = pathText.split('/');
            pathArray.pop();
            pathArray[0] = "children.";
            for(var i=1; i<pathArray.length; i++){
                var lastObj = pathArray.length-1;
                if(pathArray[i] !== "children."){
                    if( lastObj !== i && pathArray[i] !== ".children."){
                        pathArray.splice(i+1,0,".children.");
                        i++;
                    }
                }
            }
        ...
        redraw: function(TREE) {
            return function() {
                ...
                if (d3.event.sourceEvent.type==='mousewheel' || d3.event.sourceEvent.type==='DOMMouseScroll' || d3.event.sourceEvent.type==='wheel'){
                    var event = new CustomEvent("zoomTree", {detail:{data:TREE}});
                    TREE.view.getElement().element.dispatchEvent(event);
                }
                else{
         ...
         * @param noOfOpenNodes
         */
        nodeSpace: function (noOfOpenNodes) {
